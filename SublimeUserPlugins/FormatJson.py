...
def run_async(select, view):
	...
	try:
		...
		''').lstrip()  # yapf: disable
		view.add_regions(
			"format_error", [sublime.Region(view.line(view.sel()[0]).begin()+1)],
			scope="invalid",
			annotations=[
				annotation.replace('__content__', "Invalid Syntax")
			],
			flags=(sublime.DRAW_NO_FILL | sublime.DRAW_NO_OUTLINE),
			on_close=lambda:
			(view.erase_regions("format_error"), view.hide_popup())
		)
		sublime.status_message(
			"FormatJson's dependency 'yapf' ran into an error while trying to format :'("
		)
